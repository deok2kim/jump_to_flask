from flask import Blueprint, request, g, url_for, render_template, flash
...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(f"{url_for('question.detail', question_id=question_id)}#comment_{comment.id}")
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        db.session.add(comment)
        db.session.commit()
        return redirect(f"{url_for('question.detail', question_id=answer.question.id)}#comment_{comment.id}")
...
def modify_answer(comment_id):
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit():
            ...
            comment.modify_date = datetime.now()
            db.session.commit()
            return redirect(f"{url_for('question.detail', question_id=comment.answer.question.id)}#comment_{comment.id}")
